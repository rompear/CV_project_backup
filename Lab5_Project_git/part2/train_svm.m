...
best = train(double(data.trainset.labels), data.trainset.features, '-C -s 0');
model = train(double(data.trainset.labels), data.trainset.features, sprintf('-c %f -s 0', best(1))); % use the same solver: -s 0
